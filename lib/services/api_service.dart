...
class ApiService {
  // Remove this line - use apiBaseUrl from globals.dart instead
  final String baseUrl = "http://localhost:62366";

  Future<List<Account>> getAllAccounts() async {
    final response = await http.get(
      Uri.parse('$apiBaseUrl/account/all_accounts'),  // Fixed: use apiBaseUrl
      headers: {
    ...
    if (response.statusCode == 200) {
      print('Received JSON for accounts: ${response.body}');
      final List<dynamic> jsonData = jsonDecode(response.body) as List<dynamic>;

      List<Account> accounts = jsonData
